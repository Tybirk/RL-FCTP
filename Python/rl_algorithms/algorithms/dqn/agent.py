...
class DQNAgent(Agent):
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        if self.total_step < self.hyper_params["BUFFER_SIZE"]:
    ...
    def train(self, pre_train_episodes = None):
        """Train the agent."""
        # logger
        ...
        for self.i_episode in range(1, self.args.episode_num + 1):
            ...
            t_begin = time.time()
            if pre_train_episodes is not None:
                step_generator = self.env.IRNLS(100000, self.env.name)

            while not done:
                if self.args.render and self.i_episode >= self.args.render_after:
                    self.env.render()

                if pre_train_episodes is not None and self.i_episode < pre_train_episodes:
                    self.curr_state = state
                    action, next_state, reward, done, _ = next(step_generator)
                    done_bool = (
                        False if self.episode_step == self.args.max_episode_steps else done
                    )
                    transition = (self.curr_state, action, reward, next_state, done_bool)
                ...
                state = next_state
                score += reward
            print("terminated at step ", self.episode_step)
            t_end = time.time()
