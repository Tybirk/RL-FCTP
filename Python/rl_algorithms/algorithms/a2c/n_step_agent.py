# -*- coding: utf-8 -*-
"""n-Step Advantage Actor-Critic agent for episodic tasks in OpenAI Gym.
...
class A2CAgent(Agent):
    """n-Step Advantage Actor-Critic interacting with environment.
    ...
    def __init__(
        ...
        self.episode_step = 0
        self.i_episode = 0
        self.gamma_array = np.array([self.hyper_params["GAMMA"] ** i for i in range(n_step)])
    ...
    def select_action(self, state: np.ndarray) -> torch.Tensor:
        ...
        # print(torch.FloatTensor(state).is_cuda)
    ...
    def update_model(self, reward_memory) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def train(self):
        ...
        for self.i_episode in range(1, self.args.episode_num + 1):
            ...
            reward_memory = np.zeros(self.n_step)
            state_memory = deque()
            log_probs = deque()
            action_memory = deque()
            ...
            while tau < terminal_step - 1:
                ...
                if self.args.render and self.i_episode >= self.args.render_after:
                    self.env.render()
                reward = 0  # Used in terminal steps
                if self.episode_step < terminal_step:
                    action = self.select_action(state)
                    action_memory.append(action)
                    next_state, reward, done = self.step(action)
                    if done:
                        terminal_step = self.episode_step

                reward_memory[:-1] = reward_memory[1:]
                reward_memory[-1] = reward
                ...
                if tau >= 0:
                    ...
                    # Q_t   = r + gamma * V(s_{t+1})  if state != Terminal
                    #       = r                       otherwise
                    next_value = self.critic(next_state).cpu().detach()

                    G = self.gamma_array @ reward_memory
                    if tau + self.n_step < terminal_step:
                        G += self.gamma_array[-1] * next_value


                    state_tau = torch.FloatTensor(state_memory.popleft()).to(device)
                    _, dist = self.actor(state_tau)  # current prob of old action
                    selected_action = action_memory.popleft()
                    pred_value = self.critic(state_tau)
                    G = torch.FloatTensor([G])
                    G = G.to(device)
                    log_prob = dist.log_prob(selected_action)

                    # advantage = Q_t - V(s_t)
                    advantage = G - pred_value

                    # calculate loss at the current step
                    policy_loss = -advantage.detach() * log_prob  # adv. is not backpropagated
                    policy_loss += self.hyper_params["W_ENTROPY"] * -log_prob  # entropy
                    value_loss = F.smooth_l1_loss(pred_value, G.detach())
